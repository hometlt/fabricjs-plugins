...
class Demo {
    ...
    constructor(options: DemoOptions) {
        ...
        this.fabricCanvas.on("object:placed", (e) => {
            ...
            if(e.target.constructor === fabric.Table){
                let target = e.target as fabric.Table;
                target.set({
            ...
            else if(e.target.constructor === fabric.Rect){
                let target = e.target as fabric.Rect;
    ...
    canvasSelectZoomMenu(){
        this.wrapperMenuEl = document.getElementById("wrapper-menu")
    ...
    makeDemoTable(){
        let demo : {objects?: fabric.IObjectOptions[]} = JSON.parse(localStorage.getItem('demo'));
     ...
     */
    makeMainMenu(){
        this.buttonsMenuEl = document.getElementById("buttons-menu")
        this.buttonsMenuEl.addEventListener("click", event =>  {
            let element = event.target as Element
            switch (element.id) {
                case "draw-rect": {
                    let btn = document.getElementById("draw-rect")
                    if(this.fabricCanvas.selection){
                        this.fabricCanvas.enableDrawing(fabric.Rect)
                        btn.textContent = "Cancel"
                    }
                    else{
                        this.fabricCanvas.disableDrawing()
                        btn.textContent = "Draw Rect"
                    }
                    break;
                }
                case "draw-table": {
                    let btn = document.getElementById("draw-table")
                    if(this.fabricCanvas.selection){
                        this.fabricCanvas.enableDrawing(fabric.Table)
                        btn.textContent = "Cancel"
                    }
                    else{
                        this.fabricCanvas.disableDrawing()
                        btn.textContent = "Draw Table"
                    }
                    break;
                }
                case "create-table": {
                ...
                case "create-rectangle": {
                    let btn = document.getElementById("create-rectangle")
                    if(this.fabricCanvas.selection){
                        this.fabricCanvas.enableDrawing(fabric.Rect)
                        btn.textContent = "Cancel"
                    }
        ...
        this.contextMenuEl.addEventListener("click", event =>  {
            ...
            switch (element.id) {
                ...
                case "delete-table":
                    ...
                    break;
                case "insert-column":
                    target.insertColumn(target.getSelectionBounds().x2)
                    break;
                case "insert-row":
                    target.insertRow(target.getSelectionBounds().y2)
                    break;
                case "delete-column":
    ...
    _showHide(id: string,c : boolean ,displayStyle: string =  'block'){
        document.getElementById(id).style.display = c  ? displayStyle : 'none';
    }
    ...
    analyzeTable(o : fabric.Table){
        ...
        for (let y =0 ; y < cells.length ; y++) {
            ...
            for(let cell of cells[y]){
                ...
                td.style.width = cell.width * o.scaleX + 'px'
                td.style.maxWidth = cell.width * o.scaleX + 'px'
                td.style.height = cell.height * o.scaleY + 'px'
                td.style.maxHeight = cell.height* o.scaleY + 'px'
                tr.appendChild(td)
                ...
                for(let i =0 ; i< 4 ; i++ ){
                    let b = coords[i]
                    b.x = Math.round(o.left  + b.x * o.scaleX - fImage.left)
                    b.y = Math.round(o.top + b.y * o.scaleY - fImage.top)
                }
    ...
    debugSelection(canvas: fabric.Canvas){
        ...
        this.fabricCanvas.on("selection:created",this.updateSelection.bind(this))
        this.fabricCanvas.on("selection:updated",this.updateSelection.bind(this))
        this.fabricCanvas.on("selection:cleared",this.updateSelection.bind(this))
    }
...
new Demo({
    imageSrc: "./assets/images/1_FSj8PQGOpZyoirhkpXe64A.png"
})
